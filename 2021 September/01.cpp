// Author: envifly
// Problem: B. Falling Anvils from Codeforces - Codeforces Beta Round #69 (Div. 1 Only)
// URL: https://codeforces.com/problemset/problem/77/B
// Limits: 2000 ms, 256 MB
// Attempted: 2021-09-01 10:13:43 EST
...
void doStuff(){
	...
	int area = a * (2*b);
	double upper = 1.0 * a / 4;
	double left = b * a;
	double right;
	if(4*b >= a){
		right = (upper * a)/2; // intersects with x = a
	}
	else{
		right = a * b - (4*b*b)/2; // intersects with y = b
	}
	cout<<setprecision(69)<<(left+right)/area<<endl;
