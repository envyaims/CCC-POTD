// Author: envy
// Problem: B. Longest Palindrome from Codeforces - Codeforces Round #620 (Div. 2)
// URL: https://codeforces.com/problemset/problem/1304/B
// Limits: 1000 ms, 256 MB
// Attempted: 2021-09-24 19:01:33 EST
...
void doStuff(){
	...
	vector<bool> haspalidrome(n);
	vector<bool> selfpalidrome(n);
	vector<pair<string,string>> palidromes;
	
	for(int i = 0; i < n; i++){
		string rev = strs[i];
		reverse(all(rev));
		if(rev == strs[i]){
			selfpalidrome[i] = true;
		}
		if(!haspalidrome[i]){
			for(int j = i+1; j < n; j++){
				if(!haspalidrome[j] && strs[j] == rev){
					haspalidrome[j] = haspalidrome[i] = true;
					palidromes.pb(mp(strs[i],rev));
					break;
				}
			}
		}
	...
	string mid = "";
	for(int i = 0; i < n; i++){
		if(selfpalidrome[i] && !haspalidrome[i]){
			mid = strs[i];
			break;
		}
	}
	...
	string output = "";
	for(int i = 0; i < palidromes.size(); i++){
		output += palidromes[i].F;
	}
	output += mid;
	for(int i = palidromes.size() - 1; i >= 0; i--){
		output += palidromes[i].S;
	}
	print(output.size());
	print(output);
