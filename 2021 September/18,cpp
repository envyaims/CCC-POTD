// Author: envy
// Problem: Sliding Median from CSES - CSES Problem Set
// URL: https://cses.fi/problemset/task/1076/
// Limits: 1000 ms, 512 MB
// Attempted: 2021-09-18 19:40:56 EST
...
void doStuff(){
	...
	map<int,int> occur;
	for(int i = 0; i < k; i++){
		oset.insert(mp(nums[i],occur[nums[i]]));
		occur[nums[i]]++;
	}
	...
	while(bp < n){
		debug(oset);
		oset.erase(mp(nums[ap],occur[nums[ap]]-1));
		occur[nums[ap]]--;
		oset.insert(mp(nums[bp],occur[nums[bp]]));
		occur[nums[bp]]++;
		ap++;
		bp++;
		cout<<oset.find_by_order((k-1)/2)->first<<" "; 
