// Author: envy
// Problem: C. Ball in Berland from Codeforces - Codeforces Round #697 (Div. 3)
// URL: https://codeforces.com/problemset/problem/1475/C
// Limits: 2000 ms, 256 MB
// Attempted: 2021-09-05 21:13:04 EST
...
void doStuff(){
	...
	vector<int> apair(k);
	vector<int> bpair(k);
	vector<int> aedgecount(a+1);
	vector<int> bedgecount(b+1);
	readarr(apair);
	for(int i = 0; i < k; i++){
		ri(bpair[i]);
		aedgecount[apair[i]]++;
	...
	int ans = 0;
	for(int i = 0; i < k; i++){
		ans += k-(aedgecount[apair[i]] + bedgecount[bpair[i]] - 1);
	}
	print(ans/2);
