// Author: envy
// Problem: C - Doubled from AtCoder - AtCoder Beginner Contest 196
// URL: https://atcoder.jp/contests/abc196/tasks/abc196_c
// Limits: 2000 ms, 1024 MB
// Attempted: 2021-10-16 09:02:44 EST
...
void doStuff(){
	...
	int ans = stoi(string((n-1)/2,'9'));
	if(n % 2 == 0){
		int front = stoi(s.substr(0,n/2));
		int add =  front - pow(10,(n-1)/2);
		int back = stoi(s.substr(n/2));
		if(back >= front){
			ans += add+1;
		}
		else ans += add;
