// Author: envy
// Problem: A. Anu Has a Function from Codeforces - Codeforces Round #618 (Div. 1)
// URL: https://codeforces.com/problemset/problem/1299/A
// Limits: 1000 ms, 256 MB
// Attempted: 2021-10-11 14:28:22 EST
...
void doStuff(){
	...
	for(int i = 0; i < n; i++){
		int a; cin>>a;
		nums[i] = bitset<32>(a).to_string();
		val[i] = a;
	}
	for(int i = 0; i < 32; i++){
		vector<int> one, zero;
		for(int j = 0; j < n; j++){
			nums[j][i] == '0' ? zero.pb(val[j]) : one.pb(val[j]);
		}
		if(one.size() == 1){
			cout<<one.back()<<" ";
			for(int i: zero){
