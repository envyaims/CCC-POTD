// Author: envy
// Problem: B - Ice Rink Game from AtCoder - AtCoder Grand Contest 020
// URL: https://atcoder.jp/contests/agc020/tasks/agc020_b
// Limits: 2000 ms, 512 MB
// Attempted: 2021-10-03 15:24:07 EST
...
void doStuff(){
	int n; cin>>n;
	vector<int> ntarsis(n);
	for(int &i: ntarsis)cin>>i;
	int small = 2, big = 2;
	for(int i = n-1; i >= 0; i--){
		if(small % ntarsis[i] > 0){
			small += (ntarsis[i] - (small % ntarsis[i]));
		}
		big = (big / ntarsis[i] + 1) * ntarsis[i] -1;
		if(small > big){
			print(-1);
			return;
		}
