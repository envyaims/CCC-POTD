// Author: envy
// Problem: B. Orac and Models from Codeforces - Codeforces Round #641 (Div. 2)
// URL: https://codeforces.com/problemset/problem/1350/B
// Limits: 3000 ms, 256 MB
// Attempted: 2021-10-10 10:05:53 EST
...
void doStuff(){
	...
	for(int i = 1; i <= n; i++){
		for(int j = 1; j*j <= i; j++){
			if(i % j == 0){
				if(nums[i] > nums[i/j]){
					max_self(dp[i],dp[i/j]+1);
				}
				if(i/j != j and nums[i] > nums[j]){
