...
signed main(){
	...
	while(1){
		...
		if(pa[0] == pb[0] && pa[1] != pb[1]){
			...
			if(stoi(pa[1]) - stoi(pb[1]) == stoi(pa[0] )- stoi(pb[0])){
				int apos = stoi(pa[0]), bpos = stoi(pb[0]);
				if(stoi(pa[1]) - stoi(pb[1]) < 0){
					swap(pa,pb);
					swap(apos,bpos);
				}
				for(int i = 0; i <=abs(apos-bpos); i++){
					cnt[stoi(pa[0])-i][stoi(pa[1])-i]++;
				}
			}
			else if(abs(stoi(pa[1]) - stoi(pb[1]) )== abs(stoi(pa[0] )- stoi(pb[0]))){
				int apos = stoi(pa[0]), bpos = stoi(pb[0]);
				int dist = abs(apos-bpos);
				if(apos < bpos){
					for(int i = 0; i <= dist; i++){
						cnt[apos+i][stoi(pa[1])-i]++;
					}
				}
				else{
					for(int i = 0; i <= dist; i++){
