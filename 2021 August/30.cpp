...
// Memory Limit: 256 MB
// Time Limit: 2000 ms
// Attempted: 2021-08-30 07:57:09 EST
...
void doStuff(){
	...
	// first guy hit it first try: a/b
	// first guy misses, second guy misses, then first guy hits: (1-a/b) * (1*c/d) * a/b
	// next case: (1-a/b) * (1*c/d) * (1-a/b) * (1-c/d) * a/b
	// sum up infinitely?
	
	double hit = 1.0 * a/b;
	double miss = 1-(hit);
	double secondmiss = 1-(1.0 * c/d);
	double ans = hit;
